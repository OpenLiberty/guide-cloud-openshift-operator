...
The Open Liberty Operator provides a method of deploying and managing Open Liberty applications on OpenShift or OKD
(the community distribution of OpenShift) clusters. The Open Liberty Operator introduces new
https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/[Custom Resource Definitions (CRDs)^]
including the `OpenLibertyApplication`. The `OpenLibertyApplication` CRD defines a set of configurations for
deploying a Liberty-based application including the application image, number of instances, and storage settings.

The https://github.com/OpenLiberty/open-liberty-operator[Open Liberty Operator^] watches resources of the
`OpenLibertyApplication` type and creates various Kubernetes resources including `Deployments`, `Services`, and `Routes`,
depending on the configurations.
The Operator then continuously compares the current state of the resources created and the desired state of application 
deployment defined in an `OpenLibertyApplication` custom resource and reconciles them when necessary. To learn more about OpenShift
Operators, check out their https://www.openshift.com/learn/topics/operators[Operators tech topic article^].
...
There are various types of deployments, check out the official https://www.openshift.com/try[Red Hat website^]
for all the options, including free versions for you to try. The simplest option is gaining temporary free access to
OpenShift Online, their cloud offering. This guide was written and tested using OpenShift Container Platform 4.3. Ensure
you are using version 4.x, since the Dockerfiles used in this guide uses the `--chown` flag, which is incompatible with
the older versions of Docker included in pre 4.x versions.
...
*The following commands MUST be run inside your OpenShift cluster; this guide assumes that all subsequent
commands will be run from within the cluster.*
...
Ensure you are logged into your cluster as an administrator.
...
The Open Liberty Operator can be installed on OpenShift through the CLI or through the web console.

*Using the CLI*

To install the Open Liberty Operator through the CLI, follow the instructions from the
https://github.com/OpenLiberty/open-liberty-operator/blob/master/doc/user-guide.adoc#operator-installation[official GitHub repository^],
with a few notes. First, replace any use of `kubectl` with `oc`. In addition, the namespace to install and watch from
should be `guide`.
...
You see the following output which shows the CRDs installed by the Operator for use:
...
You will be making use of the `OpenLibertyApplication` resources in the upcoming sections.
...
images, push them to the internal OpenShift registry, and then deploy the images using the Open Liberty Operator.

=== Packaging the applications

Navigate to the `start` directory, then run the following commands to package the `system` and `inventory`
applications:
...
You can configure OpenShift to build your images from various methods.
...
the build inputs are your [hotspot=binary file=0]`binary` (local) files, which will be streamed to OpenShift for the build. The template specifies
...
You may need to wait some time until the build is complete. To check for this, run the following commands to view the
build logs until the `Push successful` message appears:
...
The [hotspot=sysImage file=1]`applicationImage` parameter value is the only necessary parameter, and can be set in the format
`<project-name>/<image-stream-name>[:tag]`. The parameter can also point to an image hosted on an external registry such as
Docker Hub. The [hotspot=system file=1]`system` application is configured to use the
[hotspot=sysImage file=1]`image` created from the earlier build. The operator defaults to automatically redeploying the
applications when it detects a new image pushed into the `ImageStream`, if you are using an external image, you can define
...
which is done by setting the [hotspot=expose file=1]`expose` parameter to `true`. Doing so will let the operator create and
configure a route automatically for external access to your application.
...
The `system` and `inventory` applications are not functional, as they depend on Kafka, which is deployed in the next
section.
...
and can be pulled and deployed directly from Docker Hub. Kafka relies on Zookeeper to keep track of Kafka configuration
...
The `bitnami/zookeeper:3` parameter indicates the `zookeeper` Docker Hub image to pull and deploy.
...
Give the cluster a few minutes for all applications to start up and connect with one another.
...
For example: `\http://inventory-guide.apps.lights.os.fyre.ibm.com/inventory/systems`.
...
Scaling applications up is easy with Operators. Simply update the `deploy.yaml` file with the `replica: n` parameter with
...
Wait a few minutes for everything to get ready, then run the following command to see the newly scaled up `system` pods:
