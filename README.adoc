...
Search for and install the *Open Liberty Operator* and *Strimzi Operator* with the latest version but not the stable version.
...
Update the [hotspot=bootstrap hotspot=bootstrapInv file=1]`[kafka-bootstrap-address]` of the bootstrap address variable to the
...
The [hotspot=inventory file=1]`inventory` microservice is configured
similarly to the [hotspot=system file=1]`system` microservice.
Additionally, the `inventory` microservice includes the [hotspot=service file=1]`service`
and [hotspot=expose file=1]`expose` parameters.
The [hotspot=servicePort file=1]`service.port` parameter specifies which port is exposed by the container,
...
it must be exposed by setting the [hotspot=expose file=1]`expose` parameter to `true`.
