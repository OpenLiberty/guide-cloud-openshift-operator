...
You can learn more about using OpenShift by checking out the https://openliberty.io/guides/cloud-openshift.html[Deploying microservices to OpenShift^] guide.
...
To learn more about Strimzi, visit their https://strimzi.io/[official website^].
...
If you are working within the `project` guide, you'll see an asterisk (*) beside the project `guide` in the list of projects. 
...
The Open Liberty Operator introduces newCRDs,including the `OpenLibertyApplication` CRD. 
...
From the categories on the left, you can filter to see installed operators. 
...
Apache Kafka is the messaging broker that ius used in this application. The Strimzi Operator simplifies the deployment
...
Look for the output similar to the following example:
...
Visit the `inventory` application by going to the following URL: 
`http://[HOST]/inventory/systems`

Make sure to substitute the appropriate `HOST` value. For example, using the resulting output from the command above, `inventory-guide.apps.lights.os.fyre.ibm.com` is the `HOST`.
The `HOST` is substituted in the URL, as shown in the following example:
