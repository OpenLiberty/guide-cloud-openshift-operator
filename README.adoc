...
You will learn how to deploy an application to Red Hat OpenShift 4 by using Kubernetes Operators. 
You will install the operators, deploy applications, then scale up your application. 
...
Before you install any resources, you need to create a project on your OpenShift cluster. Create a project named `guide` by
...
Look for an asterisk (*) beside the `guide` project in the list of projects to confirm that you are in the `guide` project. 
The list will appear similar to the following example:
...
When you obtained your OpenShift cluster, you should have received login information for the OpenShift web console.
...
When installing the Operators, ensure they are installed into the `guide` namespace. 
...
Look for the following output, which shows the https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/[custom resource definitions^] (CRDs) that are installed by the Open Liberty Operator:
...
The Open Liberty Operator introduces new CRDs,including the `OpenLibertyApplication` CRD. 
...
The Open Liberty Operator watches for changes to instances of the OpenLibertyApplication CRD and creates Kubernetes resources that are based on the configuration that is defined in the CRD instance.

You can use the `OpenLibertyApplication` resource in the upcoming sections.
It can also be used by using any of its shortnames such as `olapps`.

Run the following command to view all the supported API resources that are available through the Strimzi Operator:
...
Look for the following output, which shows the CRDs that are installed by the Strimzi Operator:
...
In the `status` section under `conditions` you can see a report similar to the following example, if the cluster is ready:
...
To deploy the Open Liberty `system` and `inventory` microservices, you must first package the microservices, then create and
run an OpenShift build to convert the packaged microservices into runnable container images.
...
The [hotspot=imageStream file=0]`imageStream` object kind provides an abstraction from the image in the image registry. 
...
template to create the objects for the `system` and `inventory` applications separately.
...
Next, run the following commands to view the newly created `imageStream` objects and build configurations for each microservice:
...
resources, [hotspot=system file=1]`system` and [hotspot=inventory file=1]`inventory`, which are controlled by
...
The [hotspot=system file=1]`system` microservice is configured to use the [hotspot=sysImage file=1]`image` created from the earlier build. 

One of the benefits of using `imageStream` objects is that the operator redeploys the application when it detects a new image is pushed into the `ImageStream`.
...
The [hotspot=inventory file=1]`inventory` microservice is configured similarly to the [hotspot=system file=1]`system`
microservice. Additionally, the `inventory` microservice includes the [hotspot=service file=1]`service` and
[hotspot=expose file=1]`expose` parameters. The [hotspot=servicePort file=1]`service.port` parameter specifies which port
is exposed by the container, allowing the microservice to be accessed from outside the container. To access the microservice
from outside of the cluster, it must be exposed by setting the [hotspot=expose file=1]`expose` parameter to `true`.
Exposing the microservice lets the Operator automatically create and configure routes for external access to your microservice.
...
You can also replace `OpenLibertyApplications` with its shortname `olapps`.
...
Make sure to substitute the appropriate `HOST` value. For example, using the output from the command above, `inventory-guide.apps.lights.os.fyre.ibm.com` is the `HOST`.
...
Look a JSON response that is similar to the following example: 
...
Revisit the `inventory` microservice and you will now see three instances of the `system` microservice listed in the `inventory` endpoint:
