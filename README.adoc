...
and can be pulled and deployed directly from DockerHub. Kafka relies on Zookeeper to keep track of Kafka configuration
and clusters, and the Zookeeper image is also on DockerHub.
...
DockerHub. The [hotspot=system file=1]`system` application is configured to use the
