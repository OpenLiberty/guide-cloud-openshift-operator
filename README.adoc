...
orchestrator helps you to easily scale and manage your microservices in a high-availability setup.
...
(the community distribution of OpenShift) clusters.
The Open Liberty Operator introduces new Custom Resource Definitions (CRDs) including the `OpenLibertyApplication`. The 
`OpenLibertyApplication` CRD defines a set of configurations for deploying a Liberty-based application including the application 
image, number of instances, storage settings etc. The Open Liberty Operator watches resources of the `OpenLibertyApplication` 
type and creates various Kubernetes resources such as `Deployments`, `Services`, `Routes` etc depending on the configurations. 
...
Operators, check out their https://www.openshift.com/learn/topics/operators[Operators tech topic article^]. To learn more about 
the Open Liberty Operator, check out the https://github.com/OpenLiberty/open-liberty-operator[official documentation].
...
for all the options. This guide was written and tested using OpenShift Container Platform 4.3. Ensure you are using version
4.x, since the Dockerfiles used in this guide uses the `--chown` flag, which is incompatible with the older versions of
Docker included in pre 4.x versions.
...
Note that the above command MUST be run inside your OpenShift cluster; this guide assumes that all subsequent
