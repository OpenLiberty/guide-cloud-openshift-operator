...
You will be using the `OpenLibertyApplication` object `kind` in the upcoming sections.
...
The [hotspot=sysImage hotspot=invImage file=1]`applicationImage` parameter defines what container image is deployed as part of the `OpenLibertyApplication` CRD. 
