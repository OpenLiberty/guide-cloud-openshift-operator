...
CRDs define a `kind` of object that can be used. 
The Open Liberty Operator uses the `OpenLibertyApplication` `kind`. 
...
The Open Liberty Operator watches for changes to instances of the `OpenLibertyApplication` `kind` and creates Kubernetes resources that are based on the configuration that is defined in the CRD.
...
Run the following command to view all CRDs used by the Strimzi Operator:
...
Look for the following output, which shows the CRDs that are used by the Strimzi Operator:
