...
Docker limits Anonymous and Free Docker Hub users to 100-200 image container image pull requests per 6 hours. 
...
To add a private credential, navigate to the web console and select *Workloads > Secrets* from the sidebar menu. 
...
Enter `docker.io` as the *Registry Server Address*, 
`Docker Username` as the *Username*, 
`Docker Password` as the *Password* and click *Save*. 
