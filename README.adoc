...
managing Open Liberty applications on OpenShift or OKD (the community distribution of OpenShift) clusters. The Open
...
the resources created and the desired state of application deployment and reconciles them when necessary. To learn more
about OpenShift Operators, check out their https://www.openshift.com/learn/topics/operators[Operators tech topic article^].
...
To install the Open Liberty Operator, navigate to the the web console, then to the sidebar `Operators > OperatorHub`, and
...
You see the following output which shows the custom resource definitions installed by the Operator for use:
...
You will be making use of the `OpenLibertyApplication` resource in the upcoming sections.
